  ...
  en: {
    ...
    intro: (
      <>
        ...
        a monotonic, per-run sequence of typed events. No separate UI event model gets bolted on
        later — every AG-UI, A2A, and (eventually) MCP/SSE wire projection is a pure function of
        the same stream this page documents. Two places emit it: an <code>AgentSession</code>'s own
        turn loop emits <code>RunEvent</code>, and a <code>WorkflowSupervisor</code>'s superstep
        engine emits <code>WorkflowEvent</code>. Both hand back a pull-based{" "}
        <code>stream&lt;T&gt;</code> that a caller drains with <code>.next()</code> — the same
        primitive <a href="./streaming.html">the Streaming page</a> covers for a model's own text.
      </>
    ),
    introNote: (
      <>
        <code>RunEvent</code> real since Milestone 7 Phase A; <code>WorkflowEvent</code> real since
        ADR-152; <code>stream&lt;T&gt;</code> defined in <code>core/stream.hpp</code>.
      </>
    ),
    streamingNote: (
      <>
        The event stream here and streaming.html's text stream are two separate channels that
        compose. A streamed run's <em>text</em> comes from{" "}
        <code>chat_stream()</code>/<code>stream&lt;ChatResponseUpdate&gt;</code> — that page's
        subject. A run's <em>lifecycle and progress</em> — which turn is running, which tool call
        started, whether a round suspended for approval — comes from the event stream this page
        documents. An <code>AgentSession</code> with <code>set_stream_model_calls(true)</code>{" "}
        engaged emits both at once: <code>model_delta</code> events here, and{" "}
        <code>ChatResponseUpdate</code> pushes there, over the same underlying call. See{" "}
    ...
    s1Body: (
      <>
        <code>run_event_kind</code> is a 26-value vocabulary, and it's deliberately <em>total</em>:
        every kind gets a real payload type, because later AG-UI/A2A/MCP projections need the whole
        shape to bind against, not a subset redesigned per consumer.{" "}
        <code>RunEvent::seq</code> starts at 1 for a run's first event — 0 is never real — so a
        default-constructed <code>RunEvent</code> is recognizably not one that happened.
      </>
    ),
    s1Note: (
      <>
        <code>run_event.hpp</code>'s own header discloses this gap rather than hiding it:
        AgentSession's turn loop today fires real events at its actual turn-loop boundaries
        (Run/Turn/ModelCall/StateChanged). Several kinds — most of the tool-call and sandbox
        lifecycle — have a real payload shape and no confirmed emitter yet, named honestly in the
        table below rather than left for a reader to discover by grepping. One claim in that same
        header is now stale in a good way: it lists <code>ModelDelta</code> among the kinds with no
        real producer, but ADR-034's streaming turn loop and{" "}
        <code>examples/29_agent_session_events.cpp</code> prove text deltas fire for real once{" "}
        <code>set_stream_model_calls(true)</code> is set. The header predates that wiring; the
        table below reflects what's true now.
      </>
    ),
    ...
    s2Note: (
      <>
        The example proves the event stream and the returned response agree, rather than just
        asserting it: it joins every <code>model_delta</code> event's text and checks the
        concatenation equals both the scripted client's pushed text and the final{" "}
        <code>AgentResponse</code>'s own message — the same value reached two structurally
        different ways.
      </>
    ),
    s2Link: (
    ...
    s3Note: (
      <>
        A single shared channel — every event pushing into one queue — was red-teamed and found to
        have a real liveness hazard: a channel-backed producer's <code>push()</code> deliberately
        blocks under backpressure, correct for routing/lifecycle events but wrong for a{" "}
        <code>ThreadPool</code> worker thread also needed for actual compute. Enough
        concurrently-streaming nodes stuck behind a lagging UI consumer could stall the workflow
        itself. The fix is a purpose-built, non-blocking MPSC sink (
        <code>multiplex_sink&lt;T&gt;</code>) for per-node deltas only, which drops silently on
        overflow with a diagnostic counter rather than ever blocking a compute thread. ADR-152's
        own W9 proves this adversarially: with the sink's <code>push()</code> temporarily mutated
        to spin-block, the exact same test hung and was killed by a timeout; reverted, all 35
        checks passed clean again.
      </>
    ),
    ...
    s4Note: (
      <>
        The example measures this rather than inferring it: all 9 real{" "}
    ...
    s4BridgeNote: (
      <>
        ...
        <code>ToolCall</code> that would let an outer <code>AgentSession</code> silently mis-resolve
        it. This is a concrete case of an internal suspension surfacing through a chat-shaped API
        rather than this page's own event stream — the same underlying suspension machinery as{" "}
        <a href="./streaming.html">Streaming</a>, projected onto a different consumer-facing shape.
        The wire shape, and why <code>Custom</code> beats a naive <code>ToolCall</code> encoding
        here, has its own worked section on{" "}
    ...
    s5Note: (
      <>
        ...
        participants streaming concurrently, the way examples/25's own three-worker graph does.
      </>
    ),
    ...
    s6Note: (
      <>
        <code>RunEventProjector</code> maps <code>tool_call_delta</code> to a{" "}
        <code>CustomEvent</code>, not a fabricated <code>TOOL_CALL_CHUNK</code>, because the cited
        AG-UI research record gives no field shape for one. <code>A2aStreamProjector</code>{" "}
        collapses <code>approval_requested</code> onto <code>TASK_STATE_INPUT_REQUIRED</code>{" "}
        because A2A's <code>task_state</code> enum has no distinct "confirmation" member — the
        closest real state, named as such rather than invented.
      </>
    ),
  ...
  vi: {
    ...
    intro: (
      <>
        ...
        MCP/SSE đều là một hàm thuần túy trên cùng một luồng mà trang này ghi lại. Hai nơi phát ra nó:
        vòng lặp lượt của chính một <code>AgentSession</code> phát <code>RunEvent</code>, và động cơ
        superstep của một <code>WorkflowSupervisor</code> phát <code>WorkflowEvent</code>. Cả hai đều
        trả về một <code>stream&lt;T&gt;</code> kiểu pull mà bên gọi rút cạn bằng{" "}
    ...
    introNote: (
      <>
        <code>RunEvent</code> đã thật từ Milestone 7 Phase A; <code>WorkflowEvent</code> đã thật từ
        ADR-152; <code>stream&lt;T&gt;</code> định nghĩa trong <code>core/stream.hpp</code>.
      </>
    ),
    streamingNote: (
      <>
        Luồng sự kiện ở đây và luồng văn bản của streaming.html là hai kênh tách biệt, kết hợp với
        nhau. <em>Văn bản</em> của một lần chạy có streaming đến từ{" "}
        ...
        <code>ChatResponseUpdate</code> ở đó, trên cùng một lệnh gọi bên dưới. Xem{" "}
        <a href="./streaming.html">Streaming →</a>.
      </>
    ),
    ...
    s1Body: (
      <>
        <code>run_event_kind</code> là một bộ từ vựng 26 giá trị, và nó cố ý mang tính{" "}
        <em>đầy đủ</em>: mọi loại đều có một kiểu payload thật, vì các phép chiếu AG-UI/A2A/MCP sau
        này cần toàn bộ hình dạng đó để ràng buộc vào, không phải một tập con được thiết kế lại cho
        từng bên tiêu thụ. <code>RunEvent::seq</code> bắt đầu từ 1 cho sự kiện đầu tiên của một lần
        chạy — 0 không bao giờ là thật — nên một <code>RunEvent</code> khởi tạo mặc định nhận biết
        được ngay là chưa từng xảy ra.
      </>
    ),
    s1Note: (
      <>
        Chính chú thích đầu file của <code>run_event.hpp</code> nói thẳng ra khoảng trống này, không
        giấu: vòng lặp lượt của AgentSession hôm nay phát các sự kiện thật tại đúng các ranh giới
        vòng lặp lượt (Run/Turn/ModelCall/StateChanged). Một số loại — phần lớn vòng đời tool-call
        và sandbox — có hình dạng payload thật nhưng chưa có bên phát sinh nào được xác nhận, được
        nêu tên thẳng thắn trong bảng bên dưới thay vì để người đọc tự grep ra. Một khẳng định trong
        chính header đó giờ đã lỗi thời theo hướng tốt: nó liệt kê <code>ModelDelta</code> vào nhóm
        chưa có bên phát sinh thật, nhưng vòng lặp lượt có streaming của ADR-034 và{" "}
        <code>examples/29_agent_session_events.cpp</code> chứng minh các delta văn bản phát thật một
        khi <code>set_stream_model_calls(true)</code> được bật. Header đó có trước việc nối dây này;
        bảng bên dưới phản ánh đúng sự thật hiện nay.
      </>
    ),
    ...
    s2Note: (
      <>
        Ví dụ này chứng minh luồng sự kiện và phản hồi trả về khớp nhau, không chỉ khẳng định suông:
        nó nối văn bản của mọi sự kiện <code>model_delta</code> lại và kiểm tra chuỗi ghép đó bằng cả
        văn bản mà scripted client đã đẩy lẫn chính message của <code>AgentResponse</code> cuối
        cùng — cùng một giá trị, đạt được bằng hai con đường khác nhau về cấu trúc.
      </>
    ),
    s2Link: (
      <>
    ...
    s3Note: (
      <>
        Một channel dùng chung — mọi sự kiện đều đẩy vào một hàng đợi — đã được red-team và phát
        hiện một hiểm họa liveness có thật: <code>push()</code> của một producer dựa trên channel cố
        ý chặn lại khi bị áp lực ngược, điều này đúng cho các sự kiện định tuyến/vòng đời nhưng sai
        cho một luồng worker <code>ThreadPool</code> vốn cũng cần cho việc tính toán thật. Đủ số
        node đang stream đồng thời bị kẹt sau một bên tiêu thụ UI chậm có thể làm nghẽn chính
        workflow. Bản sửa là một sink MPSC không chặn, xây riêng cho mục đích này (
        <code>multiplex_sink&lt;T&gt;</code>) chỉ dành cho delta theo từng node, nó âm thầm loại bỏ
        khi tràn kèm một bộ đếm chẩn đoán thay vì bao giờ chặn một luồng tính toán. W9 của chính
        ADR-152 chứng minh điều này đối kháng: khi <code>push()</code> của sink tạm thời bị biến đổi
        để spin-block, đúng bài kiểm thử đó bị treo và bị timeout giết chết; hoàn tác lại, cả 35
        kiểm tra pass sạch trở lại.
      </>
    ),
    ...
    s4Note: (
      <>
        Ví dụ này đo trực tiếp, không chỉ suy luận ra: cả 9 sự kiện{" "}
    ...
    s4BridgeNote: (
      <>
        ...
        giải quyết sai nó. Đây là một trường hợp cụ thể của việc một sự đình chỉ nội bộ nổi lên qua
        một hình dạng API kiểu chat, chứ không phải qua luồng sự kiện của chính trang này — cùng cơ
        chế đình chỉ bên dưới như <a href="./streaming.html">Streaming</a>, được chiếu lên một hình
        dạng hướng-tới-bên-tiêu-thụ khác. Hình dạng wire, và vì sao <code>Custom</code> tốt hơn một
        mã hóa <code>ToolCall</code> ngây thơ ở đây, có hẳn một mục riêng trên{" "}
        <a href="./workflow.html#workflow-hitl-chat-client">
    ...
    s5Note: (
      <>
        ...
        fan-out đang cùng stream, đúng như đồ thị ba worker của chính examples/25.
      </>
    ),
    ...
    <section className="section" id="events">
      <div className="container">
        ...
        <RevealGroup>
          ...
          <RevealItem>
            <ApiDiagnosticNote status="stub">{t.s1Note}</ApiDiagnosticNote>
          </RevealItem>
        ...
        <RevealGroup>
          ...
          <RevealItem>
            <ApiDiagnosticNote status="stub" kind="design">{t.s3Note}</ApiDiagnosticNote>
          </RevealItem>
          ...
          <RevealItem>
            <ApiDiagnosticNote>{t.s4Note}</ApiDiagnosticNote>
          </RevealItem>
          ...
          <RevealItem>
            <ApiDiagnosticNote kind="see also">{t.s4BridgeNote}</ApiDiagnosticNote>
          </RevealItem>
