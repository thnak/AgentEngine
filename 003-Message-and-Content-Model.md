...
```
Message  = { role, parts[], metadata, message_id, created_at }
role     = system | user | assistant | tool
Part     = Text | Reasoning | Image | Audio | Video | File | Data
         | ToolCall | ToolResult | Citation | Error | Custom
```

A message is a **sequence of parts**, never a string with attachments. This is the shape A2A uses
(`Message` → `Part[]`), the shape MCP content blocks use, and the shape every modern provider API
converged on; anything simpler loses interleaved reasoning, citations, and multi-artifact results.
...
| `ToolResult` | `{call_id, parts[], is_error, usage}` |
...
- **Parts are ordered and order is meaningful.** Reasoning interleaved with text stays interleaved.
- **Unknown parts are preserved, not dropped.** A part a provider does not understand round-trips
...
Every part carries `origin ∈ {user, assistant, tool, system, external}` and a **taint flag**.
...
- **Cross-protocol**, each surface owns its own mapping module (`protocol/*/mapping.hpp`) with a
  **round-trip test as the gate**: `internal → wire → internal` must preserve every part, including
  unknown ones (§1).

**Evolution rule:** adding a part kind is additive and must not break an older peer — unknown parts
degrade to a typed placeholder that still round-trips.
...
- **Q1** — Whether `Citation` should be a part or an annotation on `Text`. Providers disagree;
  A2A and MCP both lean annotation.
...
- **Q3** — Whether taint should be tracked at sub-string granularity (span-level) rather than
  per-part. Span-level is strictly better and materially harder.
