...
// (`include/agentengine/core/middleware.hpp`), reached through `MiddlewareModelCallGateway<Inner,
// Ms...>` (`include/agentengine/core/model_call_gateway.hpp`, ADR-036).
//
// This file was originally `test_middleware_chat_client.cpp`, proving the identical machinery
// through `MiddlewareChatClient<Inner, Ms...>` -- REMOVED 2026-08-12 along with
// `FailoverChatClient`/`ResilientChatClient` (this repo had shipped nowhere, so there was no
// deprecation-then-migration cost to justify keeping them once ADR-036 gave `AgentSession` a real,
// streaming-capable successor). `middleware.hpp`'s own `ModelCallContext`/`run_before`/`run_after`/
// `enforce_backend_tool_call_provenance` are reused VERBATIM by `MiddlewareModelCallGateway` --
// unchanged by the removal -- so every check below ports mechanically: the fake `Inner` now
// satisfies `ModelCallGatewayLike` (`capabilities()`+`call()`) instead of `ChatClient`
// (`capabilities()`+`chat_stream()`), and the wrapper under test is `MiddlewareModelCallGateway`
// instead of `MiddlewareChatClient`. No assertion's MEANING changed.
//
...
// shared_ptr for the same reason test_model_call_gateway.cpp's own ScriptedGatewayBackend does:
// MiddlewareModelCallGateway takes Inner BY VALUE, so the test's local handle and the wrapper's
// internal copy are different objects after construction.
//
// Satisfies `ModelCallGatewayLike` (chat_client.hpp: `capabilities()` + `call()`), NOT `ChatClient`
// -- `MiddlewareModelCallGateway`'s own top comment names a hand-written test fixture like this one
// as a legitimate `Inner`, no `ModelCallGateway<Primary, ...>` composition required to exercise the
...
public:
    ...
    ScriptedInner() : state_(std::make_shared<State>()) {}

    [[nodiscard]] ChatClientCapabilities capabilities() const { return {}; }

    task<result<ChatResponse>> call(ChatRequest request, EffectContext&) {
...
static_assert(ModelCallGatewayLike<ScriptedInner>,
              "ScriptedInner must satisfy ModelCallGatewayLike -- MiddlewareModelCallGateway's real "
              "Inner requirement (chat_client.hpp)");
...
int main() {
    // T1: zero middleware -- call() forwards to Inner unchanged, response passes through untouched.
    {
        ScriptedInner inner;
        inner.state().next_response.model = "test-model";
        MiddlewareModelCallGateway<ScriptedInner> client{inner};
        auto ctx = make_ctx();
    ...
    {
        ScriptedInner inner;
        MiddlewareModelCallGateway<ScriptedInner, RewriteRequestMiddleware> client{
    ...
    // proof below; here just confirm after_model runs on the ordinary success path at all).
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, TracingMiddleware> client{
    ...
    // T4: before_model short-circuits -- Inner is NEVER called.
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, ShortCircuitMiddleware> client{
    ...
    // T5: before_model denies -- call() returns the error, Inner never called.
    {
        ScriptedInner inner;
        MiddlewareModelCallGateway<ScriptedInner, DenyMiddleware> client{inner, DenyMiddleware{}};
        auto ctx = make_ctx();
        auto outcome = agentengine::test_support::run_task_sync<result<ChatResponse>>(
            client.call(ChatRequest{}, ctx));
    ...
    // after-phase backward.
    {
        ScriptedInner inner;
        auto log = std::make_shared<std::vector<std::string>>();
        MiddlewareModelCallGateway<ScriptedInner, TracingMiddleware, TracingMiddleware, TracingMiddleware>
    ...
    // T7: a middleware with ONLY after_model still gets its turn on the ordinary path.
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, AfterOnlyMiddleware> client{
    ...
    // first two get an after-phase turn (in reverse: second then first).
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, TracingMiddleware, ShortCircuitMiddleware,
                                    TracingMiddleware>
    ...
    {
        ScriptedInner inner;
        MiddlewareModelCallGateway<ScriptedInner, ThrowingMiddleware> client{inner,
                                                                              ThrowingMiddleware{}};
        auto ctx = make_ctx();
        bool threw = false;
        ...
        check(!threw, "T9: a throwing hook does NOT escape call() as a raw C++ exception");
    ...
    // vendor_structured.
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, AppendToolCallMiddleware> client{
    ...
    // exactly like fabrication (T10), not just addition.
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, MutateArgsMiddleware> client{
    ...
    // empty subset" doesn't require a name or break the chain.
    {
        ...
        MiddlewareModelCallGateway<ScriptedInner, NoopMiddleware> client{inner, NoopMiddleware{}};
        auto ctx = make_ctx();
        auto outcome = agentengine::test_support::run_task_sync<result<ChatResponse>>(
            client.call(ChatRequest{}, ctx));
    ...
    std::printf("All middleware model-call-gateway proof checks passed.\n");
    return 0;
}
