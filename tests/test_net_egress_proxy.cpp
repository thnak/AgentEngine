...
bool write_all(quark::pal::fd_t fd, std::string_view data) {
    std::size_t sent = 0;
...
// hypothetical one.
void drain_request(quark::pal::fd_t fd) {
    std::string buf;
    std::array<std::byte, 4096> chunk{};
    for (int i = 0; i < 200; ++i) {
        if (buf.find("\r\n\r\n") != std::string::npos) return;
        auto r = quark::pal::recv_some(fd, chunk.data(), chunk.size());
        if (!r) {
            if (r.error() == quark::pal::would_block()) {
...
public:
    explicit TestHttpServer(std::function<void(quark::pal::fd_t)> handler) : handler_(std::move(handler)) {
...
private:
    ...
    std::function<void(quark::pal::fd_t)> handler_;
    bool ok_ = false;
...
void handle_ok(quark::pal::fd_t fd) {
    write_all(fd, "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: 5\r\n\r\nhello");
}

void handle_flood(quark::pal::fd_t fd, std::size_t total_bytes) {
...
void handle_redirect(quark::pal::fd_t fd, std::uint16_t target_port) {
...
int main() {
    ...
    {
        TestHttpServer server([](quark::pal::fd_t fd) { handle_flood(fd, 4 * 1024 * 1024); });
    ...
    }
    {
        ...
        TestHttpServer server([](quark::pal::fd_t fd) { handle_flood(fd, 17u * 1024u * 1024u); });
    ...
    // -- C10: a 3xx response is returned as-is; the redirect target is never contacted --------------
    {
        ...
        TestHttpServer redirector([target_port](quark::pal::fd_t fd) { handle_redirect(fd, target_port); });
