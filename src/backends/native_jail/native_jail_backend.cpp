...
#include "backends/native_jail/native_jail_win32_helpers.hpp"  // widen/narrow/HandleGuard -- see that
                                                                  // header's own comment; this file is
                                                                  // where they originated, ported here
                                                                  // to close the duplication (and the
                                                                  // real double-close risk the local
                                                                  // copy of HandleGuard had -- see the
                                                                  // header's own copy-deleted, move-
                                                                  // enabled version)
// worktree_mount_fs.hpp/net_egress_proxy.hpp/relay_base64.hpp moved to
// native_jail_handle_relay.cpp, alongside the dispatch_* functions that were their only real
...
// "connect_close" worker_query kinds dispatch_worker_query (below) routes to.
//
// The nine `dispatch_*` PRIVATE STATIC methods (declarations stay in native_jail_backend.hpp,
// unchanged), their shared anonymous-namespace helpers (`deny`/`parse_open_mode`/the relay-size
// constants/the test-resolver-override seam), and `set_test_connect_resolver_override()` all now
// live in native_jail_handle_relay.cpp -- a second translation unit of this same
// `agentengine_native_jail_backend` target, not a behavior change. See that file's own top comment.
// ================================================================================================
...
// set_test_connect_resolver_override() moved to native_jail_handle_relay.cpp, alongside the
// g_test_connect_resolver_override seam it writes -- see that file's own top comment.
