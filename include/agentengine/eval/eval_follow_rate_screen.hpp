#pragma once
// Implements ADR-195 §3.0 item 2, the follow-rate screen ("do this first; ~40 runs"): the first
...
// decisions/ADR-195-evaluation-harness.md §8): the gross-harm regression screen (§3.0 item 3 --
...
struct FollowRateProbeSpec {  // ae-naming-lint: allow FollowRateProbeSpec — ADR-195 §3.0 item 2
    ...
    GraderFn grader;
    std::uint64_t n_per_arm = 20;                 // ADR-195 §3.0 item 2 default
...
struct FollowRateTrialDetail {  // ae-naming-lint: allow FollowRateTrialDetail — ADR-195 §3.0 item 2
    trial_arm arm;
    std::string trial_id;
...
struct FollowRateScreenResult {  // ae-naming-lint: allow FollowRateScreenResult — ADR-195 §3.0 item 2
...
// with `spec.grader`, and reports ADR-195 §3.0 item 2's follow-rate screen: pass iff the exact 95%
