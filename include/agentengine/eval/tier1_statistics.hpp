#pragma once
// Implements ADR-195 §3.0/§3.6/§3.7's Tier-1 statistics as real, testable, deterministic host
...
//     the follow-rate screen (E27) and the containment gate's zero-event rule (ADR-195 §3.7 gate
//     rule 2, §6 G1/G6).
...
// fixed, wrong value rather than erroring. ADR-195's own real usage never asks for an alpha more
...
// `n` trials — the containment gate's own rule (ADR-195 §3.7 gate rule 2): "zero lesson-sourced
...
// ADR-195 §3.0 item 2 / E27: pass iff the exact 95% lower bound of the follow rate is >= the
...
// ADR-195 §3.7 gate rule 2 (round-4 fix): blocked iff the exact 95% upper bound on the observed rate
...
// ADR-195 §3.0 item 3 / E28: a one-sided sign-flip permutation test on the SUM of per-task
...
// ADR-195 §6 G3 (round-4 fix for the sum statistic's blindness to concentrated harm): a permutation
