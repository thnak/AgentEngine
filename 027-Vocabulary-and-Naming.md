...
| **`ImageDigestKind`** | What an `image_digest()` actually digests — `manifest` (the descriptor a reference resolves to in a registry: an image index for a multi-platform reference, an image manifest otherwise), `config` (the image's config blob), or `unknown`. Exists because a bare `sha256:…` carries no tag saying which, and two digests of different kinds for ONE image do not match — so a consumer comparing them concludes "different image" and is wrong. `unknown` means "the backend could not ESTABLISH the kind", never "not a manifest digest"; it serializes to the EMPTY string, matching the empty-digest convention. Backends MEASURE the kind rather than infer it from which image store the operator configured. Added 2026-09-18 by ADR-176 §9 | **ours** |
