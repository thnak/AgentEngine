...
| **`ImageIdentifiedSurface`** | An opt-in REFINEMENT of `ExecutionSurface` for conformers that run commands inside a named image — adds `image()` (the reference the host configured) and `image_digest()` (what that reference resolved to, empty when not known). Deliberately a separate concept rather than two more rows in `ExecutionSurface` itself: a conformer with no image identity at all is legitimate, and requiring it to answer would put a fabricated value into a provenance record. Callers dispatch with `if constexpr`. Added 2026-09-18 with GitHub issue #80 | **ours** |
