...
| **`RawQuickstartSessionBuilder<ChatClientT, Store>`** | A `quickstart`-family builder templated directly on an already-constructed `ChatClientT` (concept-constrained via `agentengine::ChatClient`) rather than on a `quickstart::Provider`, for a caller supplying its own chat client instead of picking `Provider::openai`/`Provider::anthropic`. Deliberately duplicates `QuickstartSessionBuilder`'s fluent setters rather than sharing a common base (same already-red-teamed-class risk tradeoff `QuickstartSessionBuilder` itself documents). Added 2026-09-03 | **ours** |
...
| **`AdmittedCall`** | `admit_call()`'s return shape — the resolved `ToolDescriptor*` plus the `BoundCapability`s bound for this call, extracted out of `invoke_tool()`'s steps 1/4/7/5 (resolve, authorize+bind, approve) so the parallel-batch dispatch path (`AgentSession::dispatch_tool_calls()`) can share exactly this admission logic (`decisions/ADR-160-parallel-tool-batch-scheduler.md` §5) | **ours** |
| **`AdmittedCallOutcome`** | `run_admitted_call()`'s return shape — the normalized `ToolResult`, an optional `error`, and the result's byte size, extracted out of `invoke_tool()`'s steps 8/9/10 (invoke, normalize, account) for the same batch-dispatch sharing reason as `AdmittedCall` (ADR-160 §5). Deliberately NOT named bare `RunOutcome`: that collides with the existing, shipped `agentengine::a2a::RunOutcome`, the same collision `SandboxRunOutcome`'s own row already documents avoiding | **ours** |
| **`ConcurrencyClass`** / `concurrency_class_kind` | `partition_batch()`'s output — one scheduling unit (`sequential` / `parallel` / `exclusivity_group`) covering one or more call indices from a dispatched batch; an `exclusivity_group` class holds every member sharing one `ExclusivityGroup<Name>` name together, never one class per member (ADR-160 §5 MUST-FIX 5) | **ours** |
...
| **`Transcript`** | `std::vector<agentengine::Message>` alias for `latest_outputs_of(WorkflowResult const&)` — deliberately NOT a full multi-visit history: `WorkflowResult::partial` keeps at most one entry per executor (overwritten on revisit), so a cyclic graph (Magentic/Planner) has no per-round order left to extract; honestly named for what it actually contains. Added 2026-08-31 with the ADR-157 follow-up (issue #42) | **ours** |
| **`MagenticWorkflowBuilder<TaskMsg, ReportMsg>`** | Fluent builder wiring one manager executor plus N participant executors into a Magentic-shaped `Workflow` (switch_case routing out of the manager, direct routing back, an optional plan-signoff request port) — named to mirror `WorkflowBuilder` per §8 rule 1, since MAF's own Magentic orchestration pattern has no builder type of that name to adopt verbatim | **ours** |
| **`WorkflowChatClient`** | Lets a whole, already-initialized `rt::WorkflowSupervisor` be driven as a plain `ChatClient` — one instance serves exactly one shared `WorkflowSupervisor` (GitHub issue #35) | **ours** |
| **`multiplex_sink<T>`** (`agentengine::workflow::multiplex_sink`) | A non-blocking, bounded, multi-producer/single-consumer sink for `WorkflowSupervisor`'s per-node multiplexed live event bucket (`agent_turn_event`/`moderator_stream_delta`) — `push()` never blocks and never fails; a full queue drops the incoming item and counts it, rather than stalling a `ThreadPool` worker the way `channel_producer<T,E>`'s blocking `push()` would (ADR-152, issue #29) | **ours** |
...
| **`ImageIdentifiedSurface`** | An opt-in REFINEMENT of `ExecutionSurface` for conformers that run commands inside a named image — adds `image()` (the reference the host configured), `image_digest()` (what that reference resolved to, empty when not known) and `image_digest_kind()` (what that digest digests). Deliberately a separate concept rather than three more rows in `ExecutionSurface` itself: a conformer with no image identity at all is legitimate, and requiring it to answer would put a fabricated value into a provenance record. Callers dispatch with `if constexpr`. Added 2026-09-18 with GitHub issue #80; `image_digest_kind()` added the same day by ADR-176 §9 | **ours** |
| **`ImageDigestKind`** | What an `image_digest()` actually digests — `index` (an image index / manifest list, what a multi-platform reference resolves to), `manifest` (a single-platform image manifest), `config` (the image's config blob), or `unknown`. Exists because a bare `sha256:…` carries no tag saying which, and two digests of different kinds for ONE image do not match — so a consumer comparing them concludes "different image" and is wrong. **`index` and `manifest` are SEPARATE kinds, deliberately**: an index NAMES the manifests, so its digest can never equal any of theirs, and collapsing them would license exactly the false verdict this type exists to prevent. (This row said `manifest` covered both until 2026-09-18 — the same conflation ADR-176 §11's round found in the enum itself, surviving one edit longer than the code did.) `unknown` means "the backend could not ESTABLISH the kind", never "not a manifest digest"; it serializes to the EMPTY string, matching the empty-digest convention. Backends MEASURE the kind from a media type rather than infer it from which image store the operator configured. Added 2026-09-18 by ADR-176 §9; `index`/`manifest` both proven against a live daemon by ADR-176 §13 | **ours** |
| **`ImageDigestKindMapping`** | One row of the closed media-type→kind table (`kImageDigestKindMediaTypes`) that `image_digest_kind_from_media_type()` iterates. A named table rather than a chain of comparisons so that the test which proves the mapping can ITERATE the same list instead of hand-copying it — a copy can only catch a removal or a re-mapping, never an addition, and an addition is how a wrong kind would enter. Added 2026-09-18 by ADR-176 §15 | **ours** |
| **`ImageProvenanceReply`** | The concept a tool reply must satisfy to CARRY image provenance: `image`, `image_digest` and `image_digest_kind`, all three plain assignable `std::string`s. `MandatorySandboxProvider::stamp_image_provenance()` `static_assert`s on it, so a reply declaring `image` with an `std::optional`/`Described` spelling fails to BUILD rather than shipping a reference with nothing beside it — those spellings publish an indistinguishable `{"type":"string"}` in the schema, so no wire-level check alone can catch them. Added 2026-09-18 by ADR-176 §14/§15 | **ours** |
...
| **`ConsoleUtf8Scope`** (`agentengine::pal::ConsoleUtf8Scope`) | Puts a Windows console's output AND input code pages into UTF-8 for the life of one process, and puts them back on the way out -- the engine is UTF-8 end to end, but a console left on its OEM page decodes a model's em dash into three wrong glyphs. `Scope`, not `Guard` or `Setter`, because what it names is a span of time during which a shared, outside-this-process resource is borrowed: the code page belongs to the console the parent shell is still using. An empty no-op off Windows. Added 2026-09-12 with GitHub issue #48 | **ours** |
