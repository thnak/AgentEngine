# ADR-106 — Does `trust::hmac_sha256()` need a real Linux implementation, and if so, does giving it one accidentally widen ADR-005's deliberately Windows-only cross-process-token scope?
