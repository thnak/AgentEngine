...
| C3 (cost pool race-proof, now via SpawnPump rather than a bare busy-loop) | **CORRECT (2026-08-23 follow-up)** | `test_rt_agent_spawn.cpp` T7: 16 real `std::thread` callers `submit()` concurrently against one shared `SpawnPump`/`SpawnCostBudget` (2000-token pool, 130-token cost — exactly 15 of 16 can succeed), repeated 25 rounds with a fresh pump each round. Every round: `SpawnCostBudget::remaining()` lands EXACTLY on `pool - successes*cost` (no double-spend, no lost update), every successful mint's `child_id` is pairwise distinct, every rejection carries the real `spawn_cost_budget.exhausted` code. Clean under MSVC ASan (400 real thread launches total, zero findings). TSan itself still did not run (see §7 — unchanged toolchain limitation) — this closes the "zero concurrent-thread coverage" half of the original INCONCLUSIVE, not the TSan-specific half. |
...
- **No TSan pass at all — closed in two steps, 2026-08-23.** First follow-up: `SpawnPump` gained
  ...
  test — `tests/rt/agent_spawn/test_rt_spawn_pump_concurrency.cpp` — reproduces `SpawnPump`'s exact
